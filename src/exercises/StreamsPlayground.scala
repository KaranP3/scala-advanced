...
class Cons[+A](hd: A, tl: => MyStream[A]) extends MyStream[A] {
  ...
  val prepended = 1 #:: s = Cons(1, s)
   */
  def #::[B >: A](element: B): MyStream[B] = new Cons(element, this) // prepend
  ...
  /*
  s = new Cons(1, ?)
  mapped = s.map(_ + 1) = new Cons(2, tail.map(_ + 1))
  not evaluated until you use mapped.tail
   */
...
object StreamsPlayground extends App {
  ...
  println(startFromZero.flatMap(x =>
  ...
  [2, 3, 5, 7, 9, 11, 13... filter those not divisible by 2
